...
public class StateCensusAnalyser {
    ...
    public int loadIndiaStateData(String indianStateCsvFilePath) throws StateCensusAnalyserException.CensusAnalyserException {
        try {
            ...
            Iterator<CSVState> stateCSVIterator = csvToBean.iterator();
            Iterable<CSVState> csvIterable = () -> stateCSVIterator;
            int numOfEntries = (int) StreamSupport.stream(csvIterable.spliterator(), false).count();
            return numOfEntries;
        } catch (IOException e) {
            throw new StateCensusAnalyserException.CensusAnalyserException("File not present at the location", StateCensusAnalyserException.CensusAnalyserException.ExceptionType.CENSUS_FILE_PROBLEM);
